...
namespace SurveyBackend
{
    public class LLMTools
    {
        ...
        public string? ParseSurveyResponseToNL(string surveyRawJson, string responseRawJson)
        {
            string result = "";
            // 读取问卷结构
            ...
            // 仅取页面 2 和 3 的题目
            if (surveyJson["pages"] is not JArray pages)
                return null;

            var elementDict = pages
                .Where(p => p?["name"]?.ToString() == "2" || p?["name"]?.ToString() == "3")
                .SelectMany(p => p?["elements"] as JArray ?? [])
                .Select(e => e as JObject)
                .Where(e => e != null && e["name"] != null)
                .ToDictionary(e => e!["name"]!.ToString(), e => e!);
