            ...
            if (schemaRoot["pages"] is JArray pages)
            {
                foreach (var p in pages)
                {
                    ...
                    if (!pageMatches)
                    {
                        ...
                        else if (!string.IsNullOrEmpty(ptitle) && ptitle.IndexOf(pageFilter, StringComparison.OrdinalIgnoreCase) >= 0) pageMatches = true;
                    }
    ...
    internal class Aggregator
    {
        ...
        public Aggregator(List<QuestionMeta> metas)
        {
            ...
            foreach (var m in metas)
            {
                ...
                {
                    Counts[m.Name] = new Dictionary<string, long>();
                    // init choice labels
